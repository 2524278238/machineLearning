...
class TransformerModel(nn.Module):
    def __init__(self,  input_dim_L, hidden_dim_L, num_layers_L, output_dim_L,input_dim, output_dim, d_model, nhead, num_layers, dim_feedforward,seq_length,seq_length2, dropout):
...
model_path = "LT_model336MSE.pth"

model = TransformerModel(input_dim_L,hidden_dim_L,num_layers_L,output_dim_L,input_dim, output_dim, d_model, nhead, num_layers, dim_feedforward,seq_length,seq_length2,dropout=0.1).to(device)
# 定义优化器和损失函数
...
def train_model(model, optimizer, criterion, train_loader, val_loader, test_tensor,test_target_tensor,epochs):
    if not os.path.exists(model_path):
        ...
        with torch.no_grad():
            ...
            a2=predictions.reshape((336,9)).cpu()
            variable=8 #展示预测的是哪个变量,9-ot
            c1=list(a1[:,variable])+list(b1[:,variable])
            c2=list(a1[:,variable])+list(a2[:,variable].cpu().detach().numpy())
            plt.switch_backend('agg')
            plt.plot(list(range(432)),c1 , label='Original', alpha=0.7)
            plt.plot(list(range(432)),c2 , label='Predictions', alpha=0.7)
            plt.savefig("LT-MSE-336.jpg")
            plt.show()
