...
# 制作时间窗口
def create_sequences(data,seq_length,seq_length2):
    sequences, labels = [], []
    for i in range(len(data) - seq_length2-seq_length):
        seq = data[i:i+seq_length]
        label = data[i+seq_length:i+seq_length+seq_length2]
        #print(seq.shape)
        #print(label.shape)
...
seq_length=96
seq_length2=336
data_normalized,target_normalized=create_sequences(data_normalized,seq_length,seq_length2)
...
class TransformerModel(nn.Module):
    def __init__(self, input_dim, output_dim, d_model, nhead, num_layers, dim_feedforward, seq_length,seq_length2,dropout):
        ...
        self.seq=nn.Linear(seq_length,seq_length2)
        self.linear = nn.Linear(d_model*seq_length, output_dim*seq_length2)
    ...
    def forward(self, src):
        ...
        output=output.reshape(-1,self.seq_length2,self.output_dim)
        return output
...
model_path = "transformer_model336MAE.pth"

model = TransformerModel(input_dim, output_dim, d_model, nhead, num_layers, dim_feedforward,seq_length,seq_length2,dropout=dropout).to(device)
# 定义优化器和损失函数
...
def train_model(model, optimizer, criterion, train_loader, val_loader, test_tensor,test_target_tensor,epochs):
    if not os.path.exists(model_path):
        ...
        with torch.no_grad():
            ...
            test_date=date[train_size+val_size:train_size+val_size+seq_length+seq_length2]           
            s=200 #预测哨兵
            a1=test_tensor[s].reshape((96,7)).cpu()
            b1=test_target_tensor[s].cpu()
            predictions = model(test_tensor[s].unsqueeze(0)).to(device)
            a2=predictions.reshape((336,7)).cpu()
            print(a1.shape)
            print(b1.shape)
            print(a2.shape)
            ...
            print(len(test_date.index-s-train_size-val_size))
            print(len(c1))
            plt.plot(test_date.index-s-train_size-val_size,c1 , label='Original', alpha=0.7)
            plt.plot(test_date.index-s-train_size-val_size,c2 , label='Predictions', alpha=0.7)
            plt.savefig("transformer-MAE-336.jpg")
            plt.show()
